				...
				<table border="0" align="center" cellpadding="2" cellspacing="2" width="40%">
					<tr width="100%" bgcolor="#dfebeb">
						<td width="35%" align="left">
							<small>
								<strong>
									<xsl:value-of select="$texts/text[find='CITACOES_CONCEDIDAS_PELAS_REVISTAS']/replace" />
								</strong>
							</small>
						</td>
					</tr>
					...
					<tr width="100%" bgcolor="#dfebeb">
						<td width="35%" align="left">
							<small>
								<strong>
									<xsl:value-of select="$texts/text[find='NOS_ANOS_DE']/replace" />
								</strong>
							</small>
						</td>
					</tr>
					...
					<tr width="100%" bgcolor="#dfebeb">
						<td width="35%" align="left">
							<small>
								<strong>
									<xsl:value-of select="$texts/text[find='A_REVISTAS_PUBLICADAS_EM']/replace" />
								</strong>
							</small>
						</td>
					</tr>
                	...
                	<tr>
                		<td>
                			<font face="arial" size="2" color="#0000A0">
                            	<xsl:value-of select="$texts/text[find='TOTAL_DE_CITACOES_CONCEDIDAS']/replace" />:&#160;
                        		<xsl:value-of select="cited_journals_total" />
                			</font>
                		</td>
                	</tr>  
                </table>          
       	...
       	<tr>
       		<td colspan="5">
       			<font face="arial" size="2" color="Maroon">    
                    <xsl:value-of select="$texts/text[find='CONCEDIDAS_POR']/replace" />&#160;<b><xsl:value-of select="title"/></b>:&#160;<xsl:value-of select="sub_total"/>
       			</font>
       		</td>
       	</tr>
    </xsl:template>
			...
			<td colspan="2">
                <table border="0" align="center" cellpadding="2" cellspacing="2" width="70%">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:apply-templates select="citation_list" mode="other">
                                <xsl:with-param name="totals" select="$totals/cited_journals_total"/>
                            </xsl:apply-templates>
                        </xsl:otherwise>
                    </xsl:choose>
                </table>
		...
		**** File: 12.xsl   
		...
		<tr>
			...
			<td>
				...
				<xsl:attribute name="align">left</xsl:attribute>
				<xsl:choose>
					<xsl:when test="title_cited/link_list/link [@type='scielo']">
						<xsl:apply-templates select="title_cited/link_list/link [@type='scielo']" mode="externo" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="title_cited/label" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			...
			<td>
				...
				<xsl:attribute name="width">5%</xsl:attribute>
				<xsl:attribute name="align">left</xsl:attribute>
				<xsl:apply-templates select="title_cited/link_list/link [@type='popup']" mode="popup" />
			</td>
		</tr>
        </xsl:if>
	</xsl:template>
