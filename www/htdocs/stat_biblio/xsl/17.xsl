...
-->
                        <td><b><xsl:value-of select="$texts/text[find='column_country_second']/replace"/></b></td>
                        <xsl:for-each select="$groupYear/year_source">
                            <xsl:sort select="." data-type="number" order="ascending"/>
                            <td>
                                <b>
...
<!-- aqui -->			<xsl:if test="not(starts-with($YNG_values/text(),'sum'))">
                        <td>
                            <b>
...
-->
    ...
    <xsl:template match="cooperation" mode="group_year">
        <xsl:param name="yearGroup"></xsl:param>
